...
Original file is located at
    https://colab.research.google.com/drive/1gEARczNwHM-IBfVJr6G3yOMKhBi5uI7m
"""
...
from data.mnist_fashion import mnist_fashion
from data.mnist import mnist
from model import TrainingModel, u
...
def algorithm(mdl, x, y, gamma):
    ...
    train_acc_history = []
    test_acc_history = []
    x_, y_ = mnist() # x_, y_ = mnist_fashion()

    objective1_hist = np.zeros((mdl.n_its * 2, n))
    objective2_hist = np.zeros((mdl.n_its * 2,))
    objective3_hist = np.zeros((mdl.n_its * 2,))

    y_pred_ = foreword(0, mdl, x)
    y_pred_ = np.argmax(y_pred_[..., 0], axis=1)
    y_train = np.argmax(y[..., 0], axis=1)
    acc_train = np.count_nonzero(y_pred_ == y_train) / len(y_train)
    train_acc_history.append(acc_train)

    x_test = xp.asarray(x_[20000:20200])
    y_test = xp.asarray(y_[20000:20200])
    y_pred = foreword(0, mdl, x_test)
    y_pred = np.argmax(y_pred[..., 0], axis=1)
    y_test = np.argmax(y_test[..., 0], axis=1)
    acc_test = np.count_nonzero(y_pred == y_test) / len(y_test)
    ...
    for t in range(1, mdl.n_its):
        ...
        print("Epoch: ", t, "/", mdl.n_its, "  loss= ", loss)
        loss_history[t] = loss

        y_pred_ = foreword(t, mdl, x)
        y_pred_ = np.argmax(y_pred_[..., 0], axis=1)
        y_train = np.argmax(y[..., 0], axis=1)
        acc_train = np.count_nonzero(y_pred_ == y_train) / len(y_train)
        train_acc_history.append(acc_train)
    ...
    ax = axs[0, 0]
    ax.semilogy(np.arange(mdl.n_its)[2:], loss_history[2:], c='b', linewidth=4)
    ax.set_title("Overall loss")
    #plt.figure(dpi=100)
    #plt.tight_layout()
    #plt.savefig("OverallLoss.png")

    ax = axs[0, 1]
    for i in range(objective1_hist.shape[1]):
        ax.semilogy(np.arange(len(objective2_hist))[3:], objective1_hist[3:, i])
    ax.set_title("u objective")
    #plt.figure(dpi=100)
    #plt.tight_layout()
    #plt.savefig("uLoss.png")

    ax = axs[1, 0]
    ax.semilogy(np.arange(len(objective2_hist))[3:], objective2_hist[3:], c='b', linewidth=4)
    ...
    ax = axs[1, 1]
    ax.semilogy(np.arange(len(objective3_hist))[3:], objective3_hist[3:], c='b', linewidth=4)
    ax.set_title("Wn objective")
    #plt.figure(dpi=100)
    plt.tight_layout()
    plt.savefig("ObjLosses.png")
    ...
    plt.figure(dpi=100)
    plt.plot(np.arange(mdl.n_its)[2:], loss_history[2:], c='b', linewidth=4)
    plt.ylabel("Train Overall Loss")
    plt.xlabel("Epoch")
    plt.tight_layout()
    plt.savefig("images/OverallLoss.png")
    plt.show()

    plt.figure(dpi=100)
    for i in range(objective1_hist.shape[1]):
      plt.plot(np.arange(len(objective2_hist))[3:], objective1_hist[3:, i], linewidth=4)
    plt.title("u objective")
    plt.ylabel("Loss")
    plt.xlabel("Epoch")
    plt.tight_layout()
    plt.savefig("images/uLoss.png")
    plt.show()

    plt.figure(dpi=100)
    plt.plot(np.arange(len(objective2_hist))[3:], objective2_hist[3:], c='b', linewidth=4)
    plt.title("V Objective")
    plt.ylabel("Loss")
    plt.xlabel("Epoch")
    plt.tight_layout()
    ...
    plt.figure(dpi=100)
    plt.plot(np.arange(len(objective2_hist))[3:], objective2_hist[3:], c='b', linewidth=4)
    plt.title("Wn objective")
    plt.ylabel("Loss")
    plt.xlabel("Epoch")
    plt.tight_layout()
    plt.savefig("images/WnLoss.png")
    plt.show()

    plt.figure(dpi=100)
    line1, = plt.plot(np.arange(len(train_acc_history)), train_acc_history, c='b', linewidth=4)
    line2, = plt.plot(np.arange(len(train_acc_history)), test_acc_history, c='r', linewidth=4)
    plt.legend([line1, line2], ['train', 'test'], loc='lower right')
    plt.ylabel("Accuracy")
    plt.xlabel("Epoch")
    plt.tight_layout()
    plt.savefig("testTrainAcc.png")
    plt.show()
...
x = xp.asarray(x_[20000:20200])
y = xp.asarray(y_[20000:20200])
y_pred = foreword(n_its - 1, mdl, x)
y_pred = np.argmax(y_pred[..., 0], axis=1)
y = np.argmax(y[..., 0], axis=1)
acc = np.count_nonzero(y_pred == y) / len(y)
